...
(function () {
   ...
   setTimeout (function () {
      ...
      var multiBenchmark = function (times, iterations) {
         ...
         var output = 'daleArray: '     + ((result.aDale    / result.aFor) + '').slice (0, 5) + 'x\n';
         output    += 'daleObject: '    + ((result.oDale    / result.oFor) + '').slice (0, 5) + 'x\n';
         output    += 'daleObjectOwn: ' + ((result.oDaleObj / result.oFor) + '').slice (0, 5) + 'x\n';
         output    += iterations + ' iterations, ' + Times + ' times.';
         if (! isNode) alert (output);
         dale.clog (output);
      }
